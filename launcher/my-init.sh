...
sleep 60
docker run --name scheduler-web  -e LOG_LEVEL=debug  -v /var/log/:/var/log/ -e AWS_ACCESS_KEY_ID='%s' -e AWS_SECRET_ACCESS_KEY='%s' -d -p 80:80 --link scheduler-db:mysql enyamada/scheduler-web:1.1
touch /tmp/fin
